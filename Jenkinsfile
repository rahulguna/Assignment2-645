...
stage('Deploy to K8s'){
    ...
    sh "scp -o StrictHostKeyChecking=no services.yml node-app-pod.yml ec2-user@18.206.230.82/home/ubuntu/"
     script{
      try{
        sh "ssh ec2-user@18.206.230.82 kubectl apply -f ."
      }catch(error){
        sh "ssh ec2-user@18.206.230.82 kubectl create -f ."
      }
    }
   }
 }
