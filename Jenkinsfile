...
stage('Deploy to K8s'){
    ...
    sh "scp -o StrictHostKeyChecking=no services.yml pods.yml ubuntu@54.227.168.38:/home/ubuntu/"
     script{
      try{
        sh "ssh ubuntu@54.227.168.38 kubectl apply -f ./home/ubuntu/"
      }catch(error){
        sh "ssh ubuntu@54.227.168.38 kubectl create -f ./home/ubuntu/"
      }
    }
   }
 }
